                ...
                <div class="row">
				  <div class="col-lg-12">
					...
					<table id="example1" class="table table-middle dataTable table-bordered table-condensed table-hover">
                            ...
                            <tbody id="table_show">
							<?php 
							$q=$db->query("SELECT * FROM letter_in_out,user_infromation WHERE user_id=letter_user AND letter_in_out=2 AND letter_type=1 ORDER BY letter_id DESC") or die("");
							$i=0;
		...
		success:function(r_data){
			...
			var j=1;
				for(i=0; i<r_data.length; i++){
					var s_data="<tr><td>"+j+"</td><td>"+r_data[i].letter_id+"</td><td>"+r_data[i].letter_s_name+"</td><td>"+r_data[i].letter_sub+"</td><td>"+r_data[i].letter_add+"</td><td>"+r_data[i].user_name+"</td><td>"+r_data[i].date+"</td><td><button class='btn btn-sm btn-danger btn_delete'id="+r_data[i].id+">Delete</button></td></tr>";					
